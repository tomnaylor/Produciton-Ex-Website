...
For my MS1, I wanted to create a front-end sales website for a web app I designed for a festival my employer host bi-annually. The website is aimed as a sales tool to provide details, examples, prices and user stories to the potential client before they get in touch to enquire about their specific needs.
...
* Provide a professional and flawless UX to promote confidence in the software itself.
...
#### Navigation
I have tried to make a intuitive, accessible and reactive navigation bar that stays useful across different screen sizes and devices. For desktop the navigation has at its centre the logo which also acts as the home link. On Table the logo hides to leave space for the actual links, and on mobile the entire menu is hidden and a hamburger icon provides a button to toggle the menu on and off.
...
#### Sign up buttons
The sign up buttons are seen throughout the site as the main call to action (and the end goal for the website). The buttons and text link in the footer all trigger the sign up form modal and use JavaScript to add / remove a class to hide the form when not needed.
...
#### Pricing
The pricing boxes again share the same size as the above but like the features section they aren't fixed heights. They include an increasing battery icon to convey the scope of the requirements, an example price and an example description of the type of event that would need that degree of administration.

#### Guides
Guides work in two ways. As a prospective client they show off the software and how easy it is to use. Additionally, as a current user it keeps them visiting the website for helpful tips and guides on new features or as training for new staff on how to use the system.

#### Contact Us
I wanted to have a contact section at the bottom of the page as a means to get quick correspondence from the visitor with the aim to push sales. The form is deliberately simple with only a few fields to make sure there are minimal barriers to asking about the software.

#### Footer
The footer provides a "most recent" list of news / guides / papers etc to highlight what's new on the system. It also provides a sitemap with other links (aka. terms etc.) that aren't important to put on the main site navigation. There is also a contacts section with international phone numbers, email and social links across various platforms.
...
* Feature to dynamically highlight where you are on the page by altering the class of the respective link and stick the main navigation to the top of the viewport.
...
#### Navigation menu
I want the navigation items to have a margin around them so they stand apart; the internal links to scroll smoothly to the corresponding section; and on mobile the menu to slide out of view behind a hamburger icon.
...
The UX of the form was tested across the devices above, which included the styling of the placeholder text and the position of the label relative to the input element. All browsers and devices rendered the form in the expected way.
...
All form inputs worked as expected, the form did not submit when required inputs were empty, but the phone input allowed any character. I used the regex pattern option taken from [this site](https://martinwolf.org/before-2018/blog/2015/04/html5-telephone-input-validation/) to validate the input.
...
* **Hover client logo**
  ...
  * Changed onmouseover to two separate calls to mouseenter and mouseleave so the JavaScript function wasn't trigged on every mouse move.
...
* **Sign-up modal**
  * On mobile devices with small height screens the fixed position modal wasn't able to be scrolled. This meant that most of the form inputs were not accessible
...
* **favicon**
    * Developer tools report a 404 file missing for favicon.ico I haven't yet implemented an icon for the site.
...
The inspiration behind this website was based on work I've done in the past. Currently I work in theatre, and while we might be currently dark, it is an inspiring sector to work in. I'd like to build software that takes advantage of what I know and would very much like to release a project like this in the future.
